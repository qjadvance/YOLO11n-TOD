...
from .transMamba import TransMambaBlock, SpectralEnhancedFFN
from .EVSSM import EVS, EDFFN
...
from .srconvnet import FMABlock
...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'C3k2_ContextGuided', 'C3k2_MSBlock', 'ContextGuidedBlock_Down', 'C3k2_DLKA', 'CSPStage', 'SpaceToDepthConv',
           ...
           'C3k2_PPA', 'CSMHSA', 'SRFD', 'DFB', 'CFC_CRB', 'SFC_G2', 'CGAFusion', 'CAFM', 'CAFMFusion', 'RGCSPELAN', 'C3k2_Faster_CGLU', 'SDFM', 'PSFM',
           'C3k2_Star', 'C3k2_Star_CAA', 'C3k2_KAN', 'EIEStem', 'C3k2_EIEM', 'ContextGuideFusionModule', 'C3k2_DEConv',
           'C3k2_SMPCGLU', 'C3k2_Heat', 'SBA', 'WaveletPool', 'WaveletUnPool', 'CSP_PTB', 'GLSA', 'CSPOmniBlock', 'WTConv2d', 'C3k2_WTConv',
           ...
           'C3k2_DynamicFilter', 'C3k2_TSSA', 'A2C2f_CGLU', 'A2C2f_KAN', 'A2C2f_DFFN', 'HAFB', 'A2C2f_FRFN', 'C3k2_SAVSS', 'C3k2_MobileMamba', 'C3k2_MambaOut', 'A2C2f_DYT', 'A2C2f_CGLU_DYT', 'A2C2f_DFFN_DYT', 'C3k2_MambaOut_UniRepLK', 'C3k2_EfficientVIM', 'C3k2_EfficientVIM_CGLU',
           'CSP_MSCB_SC', 'UCSB', 'C3k2_FAT', 'A2C2f_FMFFN', 'A2C2f_FMFFN_DYT', 'C3k2_LEGM', 'C3k2_RCB', 'LoGStem', 'C3k2_LFEM', 'C3k2_SBSM', 'A2C2f_SEFN', 'A2C2f_Mona', 'C3k2_LSBlock', 'C3k2_MambaOut_LSConv', 'A2C2f_DFFN_DYT_Mona', 'C3k2_TransMamba', 'A2C2f_SEFFN', 'C3k2_EVS',
...
class SpaceToDepthConv(nn.Module):
    ...
    def forward(self, x):
        ...
        idx = torch.arange(c, device=x.device).repeat_interleave(4) * 4 + \
              torch.tensor([0, 2, 1, 3], device=x.device).repeat(c)
        x_reordered = x_unfold[:, idx, :]
        out_h, out_w = h // 2, w // 2
        x = x_reordered.view(b, c * 4, out_h, out_w)
        x = self.conv(x)
        return x
...
class ScalSeq(nn.Module):
    ...
    def forward(self, x):
        ...
        # P4特征处理：通道调整 + 上采样到P3尺寸
        p4_2 = self.conv1(p4)
        p4_2 = F.interpolate(p4_2, p3.size()[2:], mode='nearest')  # 最近邻插值上采样
        
        # P5特征处理：通道调整 + 上采样到P3尺寸
        p5_2 = self.conv2(p5)
        p5_2 = F.interpolate(p5_2, p3.size()[2:], mode='nearest')  # 最近邻插值上采样
        
        # 将2D特征图转换为3D张量，在第3个维度(深度维度)增加一个维度
        p3_3d = torch.unsqueeze(p3, -3)   # [B, C, H, W] -> [B, C, 1, H, W]
        p4_3d = torch.unsqueeze(p4_2, -3) # [B, C, H, W] -> [B, C, 1, H, W]
        p5_3d = torch.unsqueeze(p5_2, -3) # [B, C, H, W] -> [B, C, 1, H, W]
    
        combine = torch.cat([p3_3d, p4_3d, p5_3d], dim=2)  # 在深度维度上拼接三个特征图，形成3D特征体 [B, C, 3, H, W]
        conv_3d = self.conv3d(combine) # 3D卷积操作，融合跨尺度特征信息
        bn = self.bn(conv_3d)
        act = self.act(bn)
        x = self.pool_3d(act)  # 3D最大池化，在深度维度上池化 [B, C, 1, H, W]
        x = torch.squeeze(x, 2)  # 去除深度维度，恢复为2D特征图 [B, C, H, W]
...
class DFB(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.cut_path = Cut(in_channels=in_channels, out_channels=out_channels)
        self.depthwise_conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1, groups=in_channels)
        self.stride_conv = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=2, padding=1, groups=out_channels)
        self.activation = nn.GELU()
        self.bn_conv = nn.BatchNorm2d(out_channels)
        self.bn_maxpool = nn.BatchNorm2d(out_channels)
        self.max_pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.fuse_conv = nn.Conv2d(3 * out_channels, out_channels, kernel_size=1, stride=1)

    def forward(self, x):
        identity = x
        dw_out = self.depthwise_conv(x)
        cut_out = self.cut_path(identity)
        stride_out = self.stride_conv(dw_out)
        stride_out = self.activation(stride_out)
        stride_out = self.bn_conv(stride_out)
        pool_out = self.max_pool(dw_out)
        pool_out = self.bn_maxpool(pool_out)
        concat_out = torch.cat([cut_out, stride_out, pool_out], dim=1)
        out = self.fuse_conv(concat_out)
        return out
...
class PartiallyTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        conv_out = self.c_b(x[:, :self.c_ch])
        trans_out = self.t_b(x[:, self.c_ch:])
        
        x_combined = torch.cat([conv_out, trans_out], dim=1)
        
        return self.conv_fuse(x_combined)
...
class CSPOmniBlock(nn.Module):
    ...
    def forward(self, x):
        y = self.cv1(x)
        c1 = int(self.cv1.conv.out_channels * self.e)
        c2 = int(self.cv1.conv.out_channels * (1 - self.e))
        ok_branch, identity = torch.split(y, [c1, c2], dim=1)
...
class C3k_HFERB(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(HFERB(c_) for _ in range(n)))

class C3k2_HFERB(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_HFERB(self.c, self.c, 2, shortcut, g) if c3k else HFERB(self.c) for _ in range(n))
...
class C3k_DTAB(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(DTAB(c_) for _ in range(n)))

class C3k2_DTAB(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_DTAB(self.c, self.c, 2, shortcut, g) if c3k else DTAB(self.c) for _ in range(n))
...
class C3k_ETB(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(ETB(c_) for _ in range(n)))

class C3k2_ETB(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_ETB(self.c, self.c, 2, shortcut, g) if c3k else ETB(self.c) for _ in range(n))
...
class C3k_FDT(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(FDT(c_) for _ in range(n)))

class C3k2_FDT(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_FDT(self.c, self.c, 2, shortcut, g) if c3k else FDT(self.c) for _ in range(n))
...
class C3k_EfficientVIM_CGLU(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(EfficientViMBlock_CGLU(c_) for _ in range(n)))

class C3k2_EfficientVIM_CGLU(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_EfficientVIM_CGLU(self.c, self.c, n, shortcut, g) if c3k else EfficientViMBlock_CGLU(self.c) for _ in range(n))
...
# RollChannelMix 模块：
# 本研究提出了一种轻量级特征混合模块 RollChannelMix，旨在通过通道分割与空间偏移操作增强特征表达能力。
...
# 因此，RollChannelMix 适用于对计算资源受限的任务，如嵌入式视觉系统或实时目标检测等场景。
class RollChannelMix(nn.Module):
    def __init__(self, shift_size):
        super(RollChannelMix, self).__init__()
        self.shift = shift_size

    def forward(self, x):
        parts = x.chunk(4, dim=1)
        part1 = torch.roll(parts[0], self.shift, dims=2)
        part2 = torch.roll(parts[1], -self.shift, dims=2)
        part3 = torch.roll(parts[2], self.shift, dims=3)
        part4 = torch.roll(parts[3], -self.shift, dims=3)
        return torch.cat([part1, part2, part3, part4], 1)

class UCSB(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1):
        super(UCSB, self).__init__()
        self.in_ch = in_channels
        self.out_ch = in_channels
        self.upsample_dwconv = nn.Sequential(
            nn.Upsample(scale_factor=2),
            Conv(self.in_ch, self.in_ch, kernel_size, g=self.in_ch, s=stride)
        )
        self.pointwise_conv = nn.Conv2d(self.in_ch, self.out_ch, 1, 1, 0, bias=True)
        self.mix = RollChannelMix(1)

    def forward(self, x):
        x = self.upsample_dwconv(x)
        b, c, h, w = x.shape
        g = self.in_ch
        cp = c // g
        x = x.view(b, g, cp, h, w)
...
class MSCB_SC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_sizes=[1,3,5], stride=1, expansion_factor=2, dw_parallel=True, add=True):
        ...
        self.RollChannelMix = RollChannelMix(1)
...
class C3k_SBSM(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(SBSM(c_) for _ in range(n)))

class C3k2_SBSM(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_SBSM(self.c, self.c, n, shortcut, g) if c3k else SBSM(self.c) for _ in range(n))
...
class C3k_EVS(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(EVS(c_) for _ in range(n)))

class C3k2_EVS(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_EVS(self.c, self.c, n, shortcut, g) if c3k else EVS(self.c) for _ in range(n))
...
######################################## DSA: Deformable Spatial Attention start ########################################

class C3k_DSAN(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(DSAN(c_) for _ in range(n)))

class C3k2_DSAN(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_DSAN(self.c, self.c, 2, shortcut, g) if c3k else DSAN(self.c) for _ in range(n))
...
######################################## DSA: Deformable Spatial Attention end ########################################
...
class C3k_RMB(C3k):
    def __init__(self, c1, c2, n=1, size=None, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(RMB(c_, size, shift_size=(0 if _ % 2 == 0 else 2)) for _ in range(n)))

class C3k2_RMB(C3k2):
    def __init__(self, c1, c2, n=1, size=None, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_RMB(self.c, self.c, 2, size, shortcut, g) if c3k else RMB(self.c, size, shift_size=(0 if _ % 2 == 0 else 2)) for _ in range(n))
...
######################################## Fourier Convolution Block with global receptive field for MRI reconstruction start ########################################
...
class FourierConv(nn.Module):
    def __init__(self, inc, ouc, size, s=1) -> None:
        super().__init__()

        self.deepsparse = DeepSparse(inc, size if s == 1 else [i * 2 for i in size])
        self.conv = Conv(inc, ouc, 1 if s == 1 else 3, s=s)
...
######################################## Fourier Convolution Block with global receptive field for MRI reconstruction end ########################################
...
class C3k_UniConvBlock(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(UniConvBlock(c_) for _ in range(n)))

class C3k2_UniConvBlock(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_UniConvBlock(self.c, self.c, 2, shortcut, g) if c3k else UniConvBlock(self.c) for _ in range(n))

######################################## ICCV2025 UniConvNet end ########################################
        
######################################## ACM MM 2025 Mobile U-ViT start ########################################

class C3k_LGLB(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(LGLBlock(c_) for _ in range(n)))

class C3k2_LGLB(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_LGLB(self.c, self.c, 2, shortcut, g) if c3k else LGLBlock(self.c) for _ in range(n))

######################################## ACM MM 2025 Mobile U-ViT end ########################################
        
######################################## ICCV2025 ConverseNet start ########################################

class C3k_ConverseB(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(ConverseBlock(c_, c_, 3) for _ in range(n)))

class C3k2_ConverseB(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_ConverseB(self.c, self.c, 2, shortcut, g) if c3k else ConverseBlock(self.c, self.c, 3) for _ in range(n))
...
class C3k_FMABlock(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(FMABlock(c_) for _ in range(n)))

class C3k2_FMABlock(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(C3k_FMABlock(self.c, self.c, 2, shortcut, g) if c3k else FMABlock(self.c) for _ in range(n))
